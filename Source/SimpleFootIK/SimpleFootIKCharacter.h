...
#include "SimpleFootIKCharacter.generated.h"

UCLASS(config=Game)
class ASimpleFootIKCharacter : public ACharacter
{
	GENERATED_BODY()
...
protected:
	...
	float GetHitDistance(FName Socket);
	float IKFootTrace(FName Socket);
	float FInterp(float CurrentValue, float TargetValue);
};
